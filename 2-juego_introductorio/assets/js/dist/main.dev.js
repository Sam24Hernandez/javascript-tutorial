...
var suposiciones = document.querySelector('.suposiciones');
var ultimoResultado = document.querySelector('.ultimoResultado');
var bajoOalto = document.querySelector('.bajoOalto'); // Almacenan referencias a la entrada del texto y al boton de "Enviar respuesta"

var adivinaSubmit = document.querySelector('.adivinaSubmit');
var adivinaField = document.querySelector('.adivinaField'); // Almacenan un conteo de intentos desde 1

var adivinaCount = 1; // Boton de reinicio 

var reiniciarButton; // Efoca el input, una vez se recarga la página
...
function verificarAdivinanza() {
  ...
  */
  var userNuevo = Number(adivinaField.value); // Bloque de código condicional:
  ...
  */
  ...
  if (userNuevo === randomNumber) {
    ...
    establecerGameOver(); // Comprueba si este intento es el ultimo turno del jugador
  } else if (adivinaCount === 10) {
    ultimoResultado.textContent = '¡Fin del juego!';
    ultimoResultado.style.backgroundColor = 'red';
    establecerGameOver(); // El jugador tiene mas intentos pero ha fallado y se muestra el mensaje.
  } else {
...
adivinaSubmit.addEventListener('click', verificarAdivinanza); // IV. Finalizando la funcionalidad del juego

function establecerGameOver() {
  // Deshabilitan el campo de texto y el boton
  adivinaField.disabled = true;
  adivinaSubmit.disabled = true; // Creando un nuevo elemento <button> con un texto dentro

  reiniciarButton = document.createElement('button');
  reiniciarButton.textContent = 'Iniciar Nuevo Juego';
  document.body.append(reiniciarButton); // Escucha eventos del nuevo boton para ejecutar el reinicio del juego
...
function reiniciarJuego() {
  ...
  adivinaCount = 1;
  var resetParas = document.querySelectorAll('.resultParas p'); // Bucle For
  ...
  adivinaField.value = '';
  adivinaField.focus();
  ultimoResultado.style.backgroundColor = 'white'; // Genera un nuevo numero al azar ¡para que no vuelvas a adivinar el mismo numero!
